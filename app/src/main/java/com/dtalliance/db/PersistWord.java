...
public class PersistWord {
	private SQLiteHelper helper;

	public PersistWord(Context context){
		helper = new SQLiteHelper(context);
	}
	
	//����ݿ�
	public PersistWord(Context context, int version){
	...
	public boolean addPersistData(String type, String title, String keyWord, String url){
		...
		String insertQuery = "insert into " + ConstantUtil.TABLE_NAME + " (type, title, keyWord, url ) values(?, ?, ?, ?)";
		boolean flag = false;
		db.beginTransaction();
		try {
	...
	public List<Remind> selectPersistData(String start, String end){
		...
		Cursor cursor = null;
		try {
			cursor = db.query(ConstantUtil.TABLE_NAME, new String[]{"title","type", "keyWord", "url"}, "_id between ? and ?",  new String[]{start, end}, null, null, null);
	...
	public List<Remind> selectAllData(){
		...
		Cursor cursor = null;
		try {
			cursor = db.query(ConstantUtil.TABLE_NAME, new String[]{"title","type", "keyWord", "url"}, null,  null, null, null, null);
			while(cursor.moveToNext()){
				Remind remind = new Remind();
				remind.setTitle(cursor.getString(0));
				remind.setType(cursor.getString(1));
	...
	public int countData(){
		...
		int count = -1;
		try {
			Cursor cursor = db.rawQuery("select count(*) from " + ConstantUtil.TABLE_NAME, null);
			while(cursor.moveToNext()){
